...
- **Model:** `claude-3-5-sonnet-20241022` (overridable via `ANTHROPIC_MODEL` env var)
- **max_tokens:** 200
- **temperature:** default (Anthropic SDK default; we don't override)
...
First draft fed the raw stack (tools / plans / seats / spend / use case) to Claude and asked it to *both* identify overspend *and* write the summary. It hallucinated:
...
The model id is plausible-looking but isn't actually a published Anthropic model. Caught on the first live test (silent fallback to template every call). Replaced with `claude-3-5-sonnet-20241022` which I verified against [Anthropic's models page](https://docs.anthropic.com/en/docs/about-claude/models). The `ANTHROPIC_MODEL` env var lets the deployment override without a code change when a newer Sonnet ships.

**Streaming the response.**
Tried `client.messages.stream(...)` so the summary would appear word-by-word on the results page. It worked but the visual effect made the report feel slower, not faster (users waited for the *whole* paragraph before sharing). Reverted to single-shot — the page renders the static summary on the server side and the audit is screenshot-ready immediately.
