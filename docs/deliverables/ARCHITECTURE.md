...
flowchart TB
  ...
  engine --> openai[generateAISummary]
  openai --> save[(Supabase audits)]
...
| AI summary | OpenAI Chat Completions (`gpt-4o-mini`) |
...
| Path | Role |
|------|------|
| `src/components/spend-form/spend-form.tsx` | Input form + localStorage |
...
| `OPENAI_API_KEY` | Optional | AI summary; template fallback if missing |
...
- **Async audit pipeline.** Today `POST /api/audit` blocks on OpenAI (~1–2s) + Supabase insert. At scale, return `202` with `auditId` immediately after the rule engine runs; enqueue OpenAI summary + DB write via Vercel Queues or Inngest. Client polls `/api/audit/[id]/status` or uses SSE.
- **Idempotency key.** Hash `(tools, teamSize, useCase)` to dedupe identical audits within 24h — cuts OpenAI cost and DB churn from refresh-spam.
...
- OpenAI at ~$0.002/audit (`gpt-4o-mini`) × 10k/day ≈ **$20/day** ($600/mo).
...
- Structured logging per route: `audit.created`, `audit.saved`, `lead.captured`, `openai.fallback`.
