...
- **Source of truth for the code:** `src/lib/pricing.ts`. This file is the human-readable mirror; if the two ever drift, the test suite (`tests/unit/pricing.test.ts`) catches the schema-level mismatch but not the dollar values, so manual cross-check is mandatory before submission.
